...
public class WebsiteMonitor {
    private static final String URL = "https://tretmstfnet0.tmis.treas.state.nj.us/";
    private static final String ELEMENT_TO_CHECK = "h1";
...
//        URL=args[0];
    ...
    public static void checkWebsiteStatus() {
        System.out.println("Checking website status...");
//        System.out.println(URL);
//        checkWebsiteAvailability(URL);
        checkSSLCertificate();
        checkHTMLElementPresence();
    }
...
//                Certificate c = session.getPeerCertificates()[0];
    ...
    public static boolean checkHTMLElementPresence() {
        try {
            Document doc = Jsoup.connect(URL).get();
            if (doc.select(ELEMENT_TO_CHECK).isEmpty()) {
                System.out.println("⚠️ Element '" + ELEMENT_TO_CHECK + "' is NOT found.");
                return false;
            } else {
                System.out.println("✅ Element '" + ELEMENT_TO_CHECK + "' is present.");
                return true;
            }
        } catch (IOException e) {
            System.out.println("⚠️ Error parsing page content: " + e.getMessage());
            return false;
        }
    }
}
